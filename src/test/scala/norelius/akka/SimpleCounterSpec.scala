...
import norelius.crdt.{GrowOnlyCounter, deserNonOsdCounter}
...
class SimpleCounterSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  val constructor = (c: GrowOnlyCounter) => new deserNonOsdCounter(c, (c: GrowOnlyCounter) => write[GrowOnlyCounter](c),
  ...
  "A OGCounter" must {
    "remove it's own reference when setting replicas" in {
      ...
      val underTest = spawn(SimpleCounter(managerProbe.ref, 0, 2, constructor,
        Config(
          sendBehavior = One(),
          sendFrequency = 10.seconds,
          finiteQueries = false)))
      val otherReplica = spawn(SimpleCounter(managerProbe.ref, 0, 2, constructor,
        Config(
          sendBehavior = One(),
          sendFrequency = 10.seconds,
          finiteQueries = false)))
    ...
    "shut down according to config" in {
      ...
      val underTest1 = spawn(SimpleCounter(managerProbe1.ref, 0, 2, constructor,
        Config(
          sendBehavior = All(),
          sendFrequency = 10.seconds,
          finiteQueries = false)))
      val underTest2 = spawn(SimpleCounter(managerProbe2.ref, 0, 2, constructor,
        Config(
          sendBehavior = All(),
          sendFrequency = 10.seconds,
          finiteQueries = true,
